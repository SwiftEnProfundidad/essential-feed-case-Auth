...
final class UserRegistrationUseCaseTests: XCTestCase {
    ...
    func test_registerUser_withValidData_whenTokenStorageFails_returnsErrorAndDoesNotStoreCredentials() async throws {
        ...
        XCTAssertEqual(returnedPersistenceSpy.tokenStorageMessages.count, 1, "Expected TokenStorage save to be attempted once") // Se intenta guardar aunque falle
        if case .save(let attemptedToken) = returnedPersistenceSpy.tokenStorageMessages.first {
    ...
    private class RegistrationPersistenceSpy: KeychainProtocol, TokenStorage, OfflineRegistrationStore {
        ...
        // TokenStorage conformance
        enum TokenStorageMessage: Equatable {
            case save(Token)
            case loadRefreshToken
            // Otros mensajes si TokenStorage tiene más métodos
        }
        var tokenStorageMessages = [TokenStorageMessage]()
        var saveTokenError: Error?

        func save(_ token: Token) async throws {
            // Es buena práctica añadir al array de mensajes ANTES de cualquier posible throw,
            // para registrar el intento.
        ...
        func loadRefreshToken() async throws -> String? {
            ...
            tokenStorageMessages.append(.loadRefreshToken) // Registrar la llamada
        ...
        var refreshTokenToReturn: String? = "default-spy-refresh-token" // Valor por defecto para el spy
        ...
        enum OfflineStoreMessage: Equatable {
            ...
            // Otros mensajes si OfflineRegistrationStore tiene más métodos
        }
    ...
    private class UserRegistrationNotifierSpy: UserRegistrationNotifier {
        ...
        private var receivedErrors: [Error] = [] // Para depuración
        ...
        func notifyRegistrationFailed(with error: Error) {
            registrationFailedError = error // Almacena el último error
            receivedErrors.append(error)    // Almacena todos los errores recibidos

            // para asegurar que cada condición se evalúe sin depender de las anteriores.
            ...
            // Comprobación específica para NetworkError.noConnectivity
        ...
        // Helper para depuración, si fuera necesario
